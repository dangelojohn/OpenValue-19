    ...
    def get_records(self):
        ...
        for workcenter in workcenters:
            for week, req in result[workcenter.id].items():
                ...
                weeks.append(week)
        for week in weeks:
            res_all = self.env["mrp.workcenter.capacity"].read_group([('workcenter_id', 'in', workcenters.ids), ('week_nro', '=', week)],
                ['workcenter_id', 'wo_capacity_requirements'], ['workcenter_id'])
            result_all = {wc: {} for wc in workcenters.ids}
            for res_group_all in res_all:
                result_all[res_group_all['workcenter_id'][0]] = res_group_all['wo_capacity_requirements']
            for workcenter in workcenters:
                check_item = self.env['mrp.capacity.check.item'].search([('workcenter_id', '=', workcenter.id), ('week_nro', '=', week)])
                if check_item:
                    check_item.write({'all_wo_capacity_requirements': result_all.get(workcenter.id, 0.0)})
...
class MrpCapacityCheckItem(models.TransientModel):
    ...
    wc_remaining_capacity = fields.Float('WC Remaining Capacity', compute='_get_wc_capacity')
    ...
    def _get_wc_capacity(self):
        for record in self:
            if record.wc_available_capacity_cal:
                record.wc_capacity_load = (record.all_wo_capacity_requirements / record.wc_available_capacity_cal) * 100
            else:
                record.wc_capacity_load = 0.0
            record.wc_remaining_capacity = record.wc_available_capacity_cal - record.all_wo_capacity_requirements
        return True
